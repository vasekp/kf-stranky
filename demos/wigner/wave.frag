...
void main(void) {
  ...
  float aval = sqrt(vBeta) * exp(-vQuadTrf*vQuadTrf/2.); // for exp(I ln(beta) (XP+PX)/2.)
  vec2 val = vec2(cos(phase), sin(phase));
  gl_FragColor = vec4(mix(color(val), vec3(1.), smoothstep(aval - .02, aval + .02, vY)), 1.);
}
